    ...
    if (data instanceof Array) {
      ...
      data.forEach(function(source) {
        if (source._id && source.type) {
          ...
          var $sourceLink = $('<a href="/api/v1/source/' + source._id + '" data-method="get">Get events for ' + source.type + ': "' + (source.keywords || source.url) + '"</a>');
          $('ul#requests').append($('<li>').addClass('source').html($sourceLink));
          new BasicRequestHandler($sourceLink);
          // Clear unread error count
          $sourceLink = $('<a href="/api/v1/source/_events/' + source._id + '" data-method="put">Clear unread error count for ' +  source.type + ': "' + (source.keywords || source.url) + '"</a>');
          $('ul#requests').append($('<li>').addClass('source').html($sourceLink));
          new BasicRequestHandler($sourceLink);
        }
      });
