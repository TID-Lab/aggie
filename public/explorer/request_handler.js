var RequestHandler = Toolbox.Base.extend({
  ...
  submit: function(params) {
    ...
    if (params.type != 'get') params.data = JSON.stringify(params.data);
    ...
    if (data instanceof Array) {
      ...
      data.forEach(function(source) {
        if (source._id && source.type) {
          ...
          var $sourceLink = $('<a href="/api/source/' + source._id + '" data-method="get">Get events for ' + source.type + ': "' + source.keywords + '"</a>');
          $('ul#requests').append($('<li>').addClass('source').html($sourceLink));
          new BasicRequestHandler($sourceLink);
          // Clear unread error count
          $sourceLink = $('<a href="/api/source/_events/' + source._id + '" data-method="put">Clear unread error count for ' +  source.type + ': "' + source.keywords + '"</a>');
          $('ul#requests').append($('<li>').addClass('source').html($sourceLink));
          new BasicRequestHandler($sourceLink);
        }
      });
