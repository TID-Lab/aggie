...
logger.error = function(error, metadata) {
  ...
  childProcess.sendToParent('log', {level: 'error', message: message, metadata: metadata});
  if (stacktrace) childProcess.sendToParent('log', {level: 'debug', message: stacktrace, metadata: metadata});
};
