...
var Mailer = function() {
  ...
  this.smtpTransport = nodemailer.createTransport(method, options);
};
...
Mailer.prototype.templates = {
  ...
  forgotPassword: {
    ...
    body: 'Greetings,<br><p>To change your password follow <a href="http://<%= host %>/password_reset/<%= token %>">this link to login</a>, or copy the link below into your browser.</p>http://<%= host %>/password_reset/<%= token %><p>You are receiving this message because someone requested a new password from <%= host %> for an account registered with this email address.</p>'
  }
};
