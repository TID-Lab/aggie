...
Mailer.prototype.reloadTransport = async function() {
  var transportConfig;
  this.config = config.get();
  var options = _.clone(this.config.email.transport.options) || {};

  const credentials = await Credentials.findById(this.config.email.transport.credentialsID).exec();
  if (!credentials) {
    logger.error(`No email credentials are selected`);
    return;
  }
