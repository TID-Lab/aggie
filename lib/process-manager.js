...
ProcessManager.prototype.fork = function(relativePath) {
  var self = this;
  absolutePath = require.resolve(path.join(process.env.PWD, relativePath));
  moduleName = path.basename(absolutePath, '.js');
...
ProcessManager.prototype.handleMessage = function(data, child) {
  switch (data.event) {
    case 'register':
      ...
      break;
    case 'log':
      logger.log(data.level, data.message, data.metadata);
      break;
    default:
