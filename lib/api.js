...
function handleRequestTimeouts(req, res, next) {
  var requestTimeout = S(config.api_request_timeout).toInt() * 1000;
...
function logRequests(req, res, next) {
  var requestLoggingEnabled = S(config.logger.api.log_requests).toBoolean();
  var responseLoggingEnabled = S(config.logger.api.log_responses).toBoolean();
...
// Add listener for mailer relevant events
settingsController.removeAllListeners('settingsUpdated:email:transport');
settingsController.removeAllListeners('settingsUpdated:email');
settingsController.addListener('settingsUpdated:email:transport', _.bind(mailer.reloadTransport, mailer));
settingsController.addListener('settingsUpdated:email', _.bind(mailer.reloadConfig, mailer));
...
app.use('/', function (req, res, next) {
  if (req.originalUrl === '/') {
     // Inject git describe version if index.html
