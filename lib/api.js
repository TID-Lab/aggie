...
app.use(require('express-spa-router')(app, {
  ...
  staticPaths: ['widget', 'js', 'css', 'templates', 'translations'],
...
app.all('/api/v\d/incident/*', auth.ensureAuthenticated);
app.all('/api/v\d/report/*', auth.ensureAuthenticated);
app.all('/api/v\d/user/*', auth.ensureAuthenticated);
app.all('/api/v\d/trend/*', auth.ensureAuthenticated);
app.all('/api/v\d/source/*', auth.ensureAuthenticated);
app.all('/api/v\d/settings/*', auth.ensureAuthenticated);
...
require('./api/v1/public-controller')(app);
