...
StatsQueryer.prototype._countReports = function(query, callback) {
  ...
  if (_.isEmpty(query)) {
    Report.estimatedDocumentCount({}, (err, count) => {
      console.log(`AAAAHHHHHHHHH: ${count}`)
      callback(err, count)
    });
  } else {
...
StatsQueryer.prototype._countIncidents = function(query, callback) {
  query || (query = {});
  Incident.countDocuments(query, callback);
};
...
StatsQueryer.prototype._countAllReports = function(callback) {
  async.parallel({
    ...
    totalReportsTagged: this._countReports.bind(this, { smtcTags: {$type: "objectId" }}),
