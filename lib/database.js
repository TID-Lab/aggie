...
mongoose.Model.findPage = function(filters, page, options, callback) {
  ...
  model.count(filters, function(err, count) {
    ...
    // if we need to populate document, we can't do an immediate
    // execution of the query. we need the promise to chain
    // into populate()
    //
    // we can pass populate('') to skip it
    model.find(filters, null, options)
      .populate(options.populate ? options.populate : '',
                options.populateKeys ? options.populateKeys : null)
