...
var ContentService = function(options) {
  ...
  this._lastFetchDate = options.lastFetchDate;
...
ContentService.prototype.fetch = function(options, callback) {
  ...
  if(!self.savedSearchFetchEnabled){
    this._doFetch({ maxCount: options.maxCount, requestType: "LISTS" }, function(responseData) {
      // Hack: Old sources send data directly, CrowdTangle sends an object with several other keys.
      const reportData = responseData.reportData || responseData;
      ...
      callback(null, self._lastReportDate, null, self._lastFetchDate);
    });
  } else{
    this._doFetch({ maxCount: options.maxCount, requestType: "SAVED_SEARCH" }, function(responseData) {
      ...
      callback(null, null, self._lastReportDateSavedSearch, self._lastFetchDate);
    });
  }
...
ContentService.prototype.getHateSpeechRequest = function(content) {
	var self = this;
	var options = { 
		method: 'POST',
