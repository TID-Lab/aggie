...
PullBot.prototype.poll = function() {
  if (!this.enabled) return;
  var self = this;
  this.fetch(async function(err, lastReportDate, lastReportDateSavedSearch, lastFetchDate) {
    if (err) {
      logger.warning(err);
    }
    ...
    if (lastFetchDate && lastFetchDate != -Infinity && self.source.lastFetchDate !== lastFetchDate) {
      self.source.lastFetchDate = lastFetchDate;
      await self.source.save();
    }
  });
};
...
PullBot.prototype.fetch = function(callback) {
  ...
  this.contentService.fetch({ maxCount: maxCount }, function(err, lastReportDate, lastReportDateSavedSearch, lastFetchDate) {
    self._fetching = false;
    callback(err, lastReportDate, lastReportDateSavedSearch, lastFetchDate);
  });
};
