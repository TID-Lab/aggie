...
const ELMOChannel = require('./channels/elmo');
const RSSChannel = require('./channels/rss');
const SMSGhChannel = require('./channels/smsgh');
...
function createChannel(source) {
    ...
    let options = {
        ...
        onFetch: async function (lastTimestamp) {
            if (this.savedSearchToggle) {
                return Source.updateOne(
                    { _id },
                    { lastReportDateSavedSearch: lastTimestamp },
                ).exec();
            } else {
                return Source.updateOne(
                    { _id },
    ...
    switch(media) {
        ...
        case 'elmo':
            options = {
                ...options,
                authToken: credentials.secrets.authToken,
                url,
            }
            channel = new ELMOChannel(options);
            break;
        case 'rss':
            options = {
                ...options,
        ...
        case 'smsgh':
            ...
            channel = new SMSGhChannel(options);
            break;
        case 'whatsapp':
            options = {
                ...options,
