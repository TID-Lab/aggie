...
/**
 * Fetches the Credentials of the given Source.
 */
async function fetchCredentials(source) {
    const { credentials: credentialsID } = source;
...
async function fetchSourceByID(src) {
    ...
    source.credentials = await fetchCredentials(source);
    return source;
}
    ...
    switch(media) {
        ...
        case 'instagram':
            options = {
                ...
                // TODO unique namespace using a secure hash of the credentials
            };
            switch(media) {
                case 'facebook':
