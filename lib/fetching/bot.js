...
Bot.prototype.start = function() {
  ...
  this.contentService.on('warning', function(warning) {
    self.source.logEvent('warning', warning.message);
  });
  this.contentService.on('error', function(error) {
    self.source.logEvent('error', error.message);
  });
};

Bot.prototype.logDrops = function() {
  if (_.all(_.values(this._logDrops))) {
    this.source.logEvent('warning', new Error('Queue full, reports being dropped. Monitor queue status for updates.'));
