...
Bot.prototype._reportListener = function(reportData) {
  if (this.enabled) {
    ...
    // If we are in experimental mode then this values may already be in the report
    // TODO need to override this and _sourceNickname for twitter's bot
    reportData._source = reportData._source || [this.source._id];
    reportData._media = reportData._media || this.source.media;
    reportData._sourceNickname = reportData._sourceNickname || [this.source.nickname];
