...
BotMaster.prototype.reloadBot = function(bot, callback) {
  var botStatus = bot.enabled;

  // _loadOne kills and reloads if bot exists
  this._loadOne(bot.source._id.toString(), callback);
}

// Reload bots when the media settings have changed
BotMaster.prototype.reloadBots = function(media) {
  var self = this;
  var botsFiltered = this._getBotsByMedia(media);
  async.each(botsFiltered, function(b,c){ self.reloadBot(b,c); }, _.noop);
}
...
BotMaster.prototype._addFetchingListeners = function(emitter) {
  ...
  _.each(mediaValues, function(media) {
    emitter.removeAllListeners('settingsUpdated:' + media);
  });
  ...
  _.each(mediaValues, function(media) {
    var event = 'settingsUpdated:' + media;
    emitter.on(event, function() {
      //If the setting is one of the media values, we reload associated bots
      if (_.contains(Source.getMediaValues(), media)) {
        self.reloadBots(media);
      }
    });
  });
};
...
// Return all bots matching the media type
BotMaster.prototype._getBotsByMedia = function(media) {
  return _.filter(this.bots, function(bot) {
    return media === bot.source.media;
  });
};
