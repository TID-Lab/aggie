...
var cronEmitter = require('../cron/ct-list-update').emitter;
...
BotMaster.prototype._addSettingsListeners = function(emitter) {
  ...
  emitter.removeAllListeners('ctListUpdated');
  ...
  emitter.on('ctListUpdated', function(data) {
    self.emit('ctListUpdated');
  });

  cronEmitter.on('ctListUpdated', function(data) {
    self.emit('ctListUpdated');
  })
};
...
BotMaster.prototype._loadAll = function(callback) {
  ...
  if (config.get().experiment) {
    ...
    Source.find({ media: { $ne: null } }, function(err, sources) {
      if (err) return callback(err);
      // Wait for bots to be added from the source:save event
...
BotMaster.prototype._add = function(bot, callback) {
  ...
  this.on('ctListUpdated', function() {
    if(bot.source.media === 'facebook' || bot.source.media === 'instagram'){
      bot.contentService.reloadCTList();
    }
  });
