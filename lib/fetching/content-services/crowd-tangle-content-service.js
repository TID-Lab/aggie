...
const { addPost } = require('../../comments');
...
var CrowdTangleContentService = function(options) {
	...
	this.endDate = config.get().crowdtangle.endDate;
...
CrowdTangleContentService.prototype._doFetch = function(options, callback) {
	...
	console.log("Crowdtangle Request : " + JSON.stringify(requestUrl));
		...
		this._httpRequest({url: requestUrl}, function(err, res, body) {
			if (err) {
				...
				if (res.statusCode === 504) {
					// Ignore Gateway Timeout error, the CT API is flaky.
				} else {
					// self.emit('erro20.42.83.622r', new Error.HTTP(res.statusCode));
				}
				return callback({"nextPageUrl": nextPageUrl, "reportData": []});
			}
			...
			SMTCTag.find({ isCommentTag: true }, (err, tags) => {
				let commentTags = [];
				if(err) {
					logger.error(err);
				}
				if (Array.isArray(tags)) {
					commentTags = tags.map((tag) => tag._id);
				}
				self._commentTags = commentTags;
...
CrowdTangleContentService.prototype._completeUrl = function(options) {
	...
	var initialStartDate = "2020-11-09T17:29:11.000Z";
...
CrowdTangleContentService.prototype._parse = function(data, options) {
	...
	const commentTags = Array.isArray(this._commentTags) ? this._commentTags : [];

	for (let i = 0; i < commentTags.length; i += 1) {
		if (!data.postUrl.match(/permalink/)) addPost(data.postUrl, () => {});
	}
