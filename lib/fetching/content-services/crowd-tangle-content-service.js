...
var CrowdTangleContentService = function(options) {
	...
	// Adding all models from folder specified by dir_models
	...
	this.langDet.setWeights([0.35, 0.4, 0.25])
		...
		this._httpRequest({url: requestUrl}, function(err, res, body) {
			...
			if (self.detectHateSpeech){
				...
				Promise.all(hateSpeechRequests.map(function(x){return x.request})).then(
					...
					callback({"nextPageUrl": nextPageUrl, "reportData": hateSpeechRequests.map(function(x){
							self._parse(x.data, options).then((res) => {
								return res
							});
							Promise.all([self._parse(x.data, options)]).then(function(res) {
								return res[0]
							});
					})});
				});
				...
				responses.map(function(x) {
					Promise.all([self._parse(x, options)]).then(function(res) {
						res.forEach(function(it, id) {
							console.log(this._cleanPost(it.metadata.description))
							console.log('########################################')
						});

						callback({"nextPageUrl": nextPageUrl, "reportData": res});
					});
				});
...
CrowdTangleContentService.prototype._cleanPost = function(tweet) {
	tweet = tweet.replace(/\n|\r/g, "");
...
CrowdTangleContentService.prototype._parse = async function(data, options) {
	var metadata = {
	...
	let cleaned_text = this._cleanPost(data.description)
