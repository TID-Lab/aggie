...
var CrowdTangleContentService = function(options) {
	...
	// Assuming we are using two models: Franc and custom-made FastText. FastText is more accurate but Franc has more coverage
	// Weights should add up to 1
	this.langDet.setWeights([0.3, 0.7])
	...
	if (requestUrl !== null){
		this._httpRequest({url: requestUrl}, function(err, res, body) {
			...
			if (self.detectHateSpeech){
				...
				var reportData = responses.map(function(x) {
...
CrowdTangleContentService.prototype._parse = async function(data, options) {
	...
	content_lang = await this.langDet.predict(content)
	let ret = {
