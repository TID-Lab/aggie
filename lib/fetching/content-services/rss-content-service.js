...
var RSSContentService = function(options) {
  ...
  this._sourceType = 'rss';
  this.botType = 'pull';
  this._lastReportDate = options._lastReportDate || null;
...
RSSContentService.prototype._doFetch = function(options, callback) {
  ...
  logger.debug('RSSContentService#fetch', this);
};
...
RSSContentService.prototype._parseStream = function(stream, callback) {
  ...
  parser.on('end', function(err) {
    ...
    _.sortBy(reports, 'authoredAt').every(function(report_data) {
      // dont cross the limit provided
      if (!self._addReport(report_data)) return false;
      
      if (report_data.authoredAt > self._lastReportDate) {
        // Store latest date for future use
        self._lastReportDate = report_data.authoredAt;
      }
      return true;
    });
    self._fetchingFinished();
    callback(self._lastReportDate);
  });
...
RSSContentService.prototype._isNew = function(report_data) {
  return !this._lastReportDate || report_data.authoredAt > this._lastReportDate;
};
