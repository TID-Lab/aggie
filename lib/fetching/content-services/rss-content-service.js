...
RSSContentService.prototype.fetch = function(options, callback) {
  ...
  this._fetchStream(function(stream) {
    self._parseStream(stream, maxCount, function(data) {
      callback(data);
    });
  });
...
RSSContentService.prototype._parseStream = function(stream, maxCount, callback) {
  var self = this;
  var reports = [];
  ...
  parser.on('end', function(err) {
    ...
    _.sortBy(reports, 'authoredAt').every(function(report_data) {
      ...
      if (maxCount > 0 && index++ > maxCount) return false;
