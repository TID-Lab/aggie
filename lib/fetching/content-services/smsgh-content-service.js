...
function SMSGhContentService(port) {
  ...
  var getKeyword = function(req) {
    return 'sms_ghana:' + req.query.keyword.toLowerCase();
  };
...
SMSGhContentService.prototype.subscribe = function(identifier) {
  if (!_.contains(this._connectedSources, identifier.toLowerCase())) {
    this._connectedSources.push(identifier.toLowerCase());
  }
  if (this._isListening) {
    return 'sms_ghana:' + identifier.toLowerCase();
  }
...
SMSGhContentService.prototype.unsubscribe = function(identifier) {
  if (_.contains(this._connectedSources, identifier.toLowerCase())) {
    this._connectedSources = _.without(this._connectedSources, identifier.toLowerCase());
  } else {
    return;
  }
