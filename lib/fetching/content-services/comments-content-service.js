...
var CommentContentService = function(options) {
    ...
    const  { count, lastFetchDate } = config.get().comments;
    this._count = count;
    if (lastFetchDate) {
...
CommentContentService.prototype._doFetch = function(_, callback) {
    ...
    const options = { body: { count: this._count } };

    if (this._lastFetchDate) {
        options.json = true;
        options.body = {
            ...options.body,
            acqAfter: this._lastFetchDate.getTime(),
            authAfter: this._lastReportDate.getTime(),
        }
    }
...
CommentContentService.prototype._parse = function(doc, callback) {
    ...
    this._lastFetchDate = new Date(doc.acquiredAt)
