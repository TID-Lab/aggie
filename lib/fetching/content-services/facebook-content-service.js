...
FacebookContentService.prototype.fetch = function(callback) {
  ...
  graph.fql({source: source, posts: posts}, function(err, res) {
    ...
    res.data.forEach(function(data) {
      switch (data.name) {
        case 'source':
          if (data.fql_result_set.length === 0) self.emit('error', new Error('Invalid group'));
          break;
        case 'posts':
          if (data.fql_result_set.length) self._handleResults(data.fql_result_set);
          return callback(null, self.lastReportDate * 1000);
      }
    });
  });
};
...
FacebookContentService.prototype._handleErrors = function(err) {
  if (err.message === 'Error processing https request') {
    ...
    this.emit('error', new Error(err.exception));
