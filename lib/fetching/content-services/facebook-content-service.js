...
FacebookContentService.prototype.fetch = function() {
  if (this.lastFetchedAt) {
    var query = {
      postsQuery: "SELECT post_id, updated_time, created_time, actor_id, permalink, message" + //, comments.comment_list.text, comments.comment_list.time, comments.comment_list.fromid" +
    ...
    var query = {
      ...
      commentsQuery: "SELECT id, fromid, time, text FROM comment WHERE" +
        " (post_id IN (SELECT post_id FROM #postsQuery)) ORDER BY time",
      //usernameQuery: 'SELECT name FROM user WHERE uid IN (SELECT actor_id FROM #postsQuery) OR uid IN (SELECT fromid FROM #commentsQuery)'
    };
  }
  this.runFQL(query);
...
FacebookContentService.prototype._buildCommentUrl = function(parent, commentId) {
  return parent.permalink + '?comment_id=' + commentId;
};
