...
var FacebookContentService = function(options) {
  ...
  this._getParams = {
    fields: 'id,name,created_time,updated_time,from,message,link,comments.order(reverse_chronological),reactions.limit(1).summary(true),likes.limit(1).summary(true),place',
...
FacebookContentService.prototype._doFetch = function(options, callback) {
  ...
  var isTransient = function(error) {
    return error.code && _.contains(transientErrorCodes, error.code);
  };
...
FacebookContentService.prototype._parse = function(data) {
  var metadata = {
    likeCount: data.likes.total_count ? data.likes.total_count : 0,
    reactionCount: data.reactions.total_count ? data.reactions.total_count : 0,
