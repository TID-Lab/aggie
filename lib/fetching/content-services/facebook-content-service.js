...
var FacebookContentService = function(options) {
  ...
  this._lastReportDate = Math.floor(options.lastReportDate  / 1000) || null;
  this._sourceType = 'facebook';
  this.botType = 'pull';
...
FacebookContentService.prototype._doFetch = function(options, callback) {
  ...
  this._loadData(options, function(err, lastReportDate){
    if (err) {
      self._handleErrors(err);
    }
    return callback(null, lastReportDate);
  });
};

// Load data from online using FQL
FacebookContentService.prototype._loadData = function(options, callback) {
  var self = this;
  ...
  if (this._lastReportDate) queries.posts += ' AND updated_time > ' + this._lastReportDate;
  ...
  graph.fql(queries, function(err, res) {
    ...
    if (res && res.data && res.data.length) {
      res.data.forEach(function(data) {
        switch (data.name) {
          case 'source':
            if (data.fql_result_set.length === 0) return callback(new Error('Invalid Facebook URL'));
            break;
          case 'posts':
            ...
            callback(null, self._lastReportDate * 1000);
            break;
        }
      });
    }
...
FacebookContentService.prototype._handleResults = function(data, authors) {
  ...
  data.every(function(post) {
    ...
    if (post.comments.comment_list.length) {
      ...
      post.comments.comment_list.forEach(function(comment) {
        // Skip old comments
        if (comment.time > self._lastReportDate) {
    ...
    if (self._canAddReports(postReports.length)) {
      postReports.forEach(function(report_data){
        self._addReport(report_data);
      });
      return true;
    }
    else {
  ...
  //emit all collected reports after sort
  self._fetchingFinished();
  this._lastReportDate = lastReportDate;
};
