...
FacebookContentService.prototype.fetch = function(options, callback) {
  ...
  graph.fql(queries, function(err, res) {
    if (err) return self._handleErrors(err);
      ...
      res.data.forEach(function(data) {
        switch (data.name) {
          ...
          case 'posts':
            ...
            if (data.fql_result_set.length) self._handleResults(data.fql_result_set, authors, maxCount);
...
FacebookContentService.prototype._handleResults = function(data, authors, maxCount) {
  var self = this;
  ...
  data.every(function(post) {
    ...
    // if these reports will exceed maxCount, return now, dont include these reports
    if (maxCount > 0 && reports.length + postReports.length > maxCount) {
      return false;
    }
    else {
