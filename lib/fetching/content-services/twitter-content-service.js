...
var blacklist = [
  /(hillary|clinton)/i, /trump/i, /jill\sstein/i,
  /Biometric\sapp/i,
  /(ㄱ|ㅅ|다)/,
  /مِن/,
  /ٱللَّه/,
...
TwitterContentService.prototype._makeReport = function(tweet) {
  ...
  if (botIds.length === 0) {
    var id = _.keys(this._connectedSources)[0];
    delete tweet._sources;
    delete tweet._sourceNicknames;
    this.emit('tweet:' + id, this._parse(tweet));
