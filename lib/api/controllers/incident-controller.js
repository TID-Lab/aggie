...
module.exports = function(app, user) {
  ...
  app.post('/api/incident', user.can('edit data'), function(req, res) {
  ...
  app.get('/api/incident', user.can('view data'), function(req, res) {
  ...
  app.get('/api/incident/:_id', user.can('view data'), function(req, res) {
  ...
  // Update an Incident
  app.put('/api/incident/:_id', user.can('edit data'), function(req, res, next) {
  ...
  app.post('/api/incident/_selected', user.can('edit data'), function(req, res) {
  ...
  app.patch('/api/incident/_tag', user.can('edit data'), function(req, res) {
    if (!req.body.ids || !req.body.ids.length) return res.send(200);
    Incident.find({ '_id': { $in: req.body.ids } }, function(err, incidents) {
      if (err) return res.send(err.status, err.message);
      if (incidents.length === 0) return res.send(200);
  ...
  app.patch('/api/incident/_untag', user.can('edit data'), function(req, res) {
    if (!req.body.ids || !req.body.ids.length) return res.send(200);
    Incident.find({ '_id': { $in: req.body.ids } }, function(err, incidents) {
      if (err) return res.send(err.status, err.message);
      if (incidents.length === 0) return res.send(200);
  ...
  app.patch('/api/incident/_clearTags', user.can('edit data'), function(req, res) {
    if (!req.body.ids || !req.body.ids.length) return res.send(200);
    Incident.find({ '_id': { $in: req.body.ids } }, function(err, incidents) {
      if (err) return res.send(err.status, err.message);
      if (incidents.length === 0) return res.send(200);
  ...
  // Delete an Incident
  app.delete('/api/incident/:_id', user.can('edit data'), function(req, res, next) {
  ...
  app.delete('/api/incident/_all', user.can('edit data'), function(req, res) {
