...
module.exports = function(app, user) {
    ...
    app.get('/api/tag', user.can('view data'), function(req, res) {
    ...
    // Create a new Tag
    app.post('/api/tag', user.can('edit tags'), function(req, res) {
    ...
    // Update a Tag
    app.put('/api/tag/:_id', user.can('edit tags'), function(req, res) {
        // Find tag to update
    ...
    // Delete a Tag
    app.delete('/api/tag/:_id', user.can('edit tags'), function(req, res) {
        SMTCTag.findById(req.params._id, function (err, tag) {
            if (err) return res.send(err.status, err.message);
            if (!tag) return res.send(404);
