...
var SocketHandler = function(app, server, auth) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    server.on('sourceErrorCountUpdated', function() {
      emitAllErrorsCount(socket);
    });

    // Send trends to client
    server.on('trend', function(trend) {
  ...
  server._addSourceListeners = function(emitter) {
    var self = this;
  ...
  server._addTrendsListeners = function(emitter) {
    var self = this;
