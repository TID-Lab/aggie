...
module.exports = function(app, server, auth) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('query', function(queryData) {
      ...
      var reportQuery = new ReportQuery(queryData);
      clientQuery = reportQuery.normalize();
      // Track query and add client as listener
      clientQuery.hash = addClient(reportQuery, socket.id);
    });
    ...
    socket.on('incidentQuery', function(queryData) {
      ...
      removeClient(clientQuery, socket.id);
      // Instantiate and normalize client query
      var incidentQuery = new IncidentQuery(queryData);
      clientQuery = incidentQuery.normalize();
      // Track query and add client as listener
      clientQuery.hash = addClient(incidentQuery, socket.id);
    });
    ...
    streamer.on('reports', function(query, reports) {
      ...
      if (ReportQuery.compare(query, new ReportQuery(clientQuery)) && queryGroups[clientQuery.hash].has(socket.id)) {
        socket.emit('reports', reports);
      }
    });

    // Send incident back to client for matching queries
    streamer.on('incidents', function(query, incidents) {
      // Determine if current client query matches the incident
      if (IncidentQuery.compare(query, new IncidentQuery(clientQuery)) && queryGroups[clientQuery.hash].has(socket.id)) {
