...
var SocketHandler = function(app, server, auth) {
  ...
  this.io.sockets.on('connection', function(socket) {
    self.clientQuery = null;
    self.emitAllErrorsCount(socket);

    self._streamerQueryListen('query', ReportQuery, socket);
    self._streamerQueryListen('incidentQuery', IncidentQuery, socket);

    self._streamerQueryCheck('reports', ReportQuery, socket);
    self._streamerQueryCheck('incidents', IncidentQuery, socket);
