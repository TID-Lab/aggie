...
module.exports = function(app, server, auth) {
  ...
  io.sockets.on('connection', function(socket) {
    ...
    function streamerQueryListen(event, QueryType) {
      socket.on(event, function(queryData) {
        ...
        clientQuery.hash = addClient(query, socket.id);
      });
    };

    // Send data back to client for matching queries
    function streamerQueryCheck(event, QueryType) {
      streamer.on(event, function(query, data) {
        // Determine if current client query matches the query
        if (QueryType.compare(query, new QueryType(clientQuery)) && queryGroups[clientQuery.hash].has(socket.id)) {
    ...
    function streamerSocketForward(event) {
    ...
    streamerQueryListen('query', ReportQuery);
    streamerQueryListen('incidentQuery', IncidentQuery);

    streamerQueryCheck('reports', ReportQuery);
    streamerQueryCheck('incidents', IncidentQuery);

    streamerSocketForward('sourceErrorCountUpdated');
    streamerSocketForward('reportStatusChanged');
    streamerSocketForward('reportIncidentChanged');
    streamerSocketForward('trend');
