...
Streamer.prototype.addListeners = function(type, emitter) {
  switch (type) {
    ...
    case 'report':
      ...
      break;
    case 'fetchingSource':
      this._addFetchingSourceListeners(emitter);
      break;
    case 'apiSource':
      this._addAPISourceListeners(emitter);
      break;
    case 'trends':
...
Streamer.prototype.query = function() {
  ...
  this.queries.forEach(function(query) {
    // Query database
    query.runQuery(function(err, results) {
...
Streamer.prototype._addIncidentListeners = function(emitter) {
  ...
  emitter.on('incident:save', function(report) {
...
Streamer.prototype._addFetchingSourceListeners = function(emitter) {
  var self = this;
  ...
  // Listens to new reports being written to the database
  emitter.on('sourceErrorCountUpdated', function(source) {
    self.emit('sourceErrorCountUpdated', source);
  });
};

Streamer.prototype._addAPISourceListeners = function(emitter) {
  var self = this;

  // Clean-up old listeners
  emitter.removeAllListeners('sourceErrorCountCleared');

  // Listens to new reports being written to the database
  emitter.on('sourceErrorCountCleared', function(source) {
    self.emit('sourceErrorCountUpdated', source);
  });
};
