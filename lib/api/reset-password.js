...
module.exports = function(app, auth) {
  ...
  function encodeToken(user) {
    ...
    var urlSafeToken = token.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    return urlSafeToken;
  }

  // Decode user token
  function decodeToken(urlSafeToken) {
    console.log('Token', urlSafeToken);
    var token = urlSafeToken.replace(/-/g, '+').replace(/_/g, '/');
