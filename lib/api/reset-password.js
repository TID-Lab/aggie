...
var PASSWORD_RESET_TIMEOUT = 86400000 * 40;
...
module.exports = function(app, auth) {
  ...
  function tokenLogin(req, res, next) {
    ...
    if (!token.username) return next(new Error.NotFound('user_not_found'));
    if (token.timestamp + PASSWORD_RESET_TIMEOUT < Date.now()) {
      return next(new Error.NotFound('login_token_expired'));
    }
