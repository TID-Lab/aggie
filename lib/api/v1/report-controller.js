...
module.exports = function(app, user) {
  ...
  // Update multiple reports in the database
  function updateReports(reports, done) {
    async.each(reports, function(item, cb){
      Report.findById(item._id, function(err, report) {
        if (err) return cb(err);
        if (!report) return cb(new Error.NotFound());
        
        //Update the actual value
        _.each(_.pick(item, ['flagged', '_incident', 'read']), function(val, key) {
          report[key] = val;
        });
        ...
        report.save(function(err, numberAffected) {
          if (err) return cb(err);
          else if (!numberAffected) return cb(new Error.NotFound());
          else return cb();
        });
      });
    }, function(err){
  ...
  app.put('/api/v1/report', user.can('edit reports'), function(req, res, next) {
    var reports = _.isArray(req.body) ? req.body : [req.body];
    updateReports(reports, function(err){
      if (err) res.send(err.status, err.message);
      else res.send(200);
    });
  });
  ...
  app.put('/api/v1/report/:_id', user.can('edit reports'), function(req, res, next) {
    ...
    updateReports([req.body], function(err){
      if (err) res.send(err.status, err.message);
      else res.send(200);
    });
  });
