      ...
      report.save(function(err, numberAffected) {
        if (err) res.send(err.status, err.message);
          ...
          var logDetails = {
            userID: req.user._id,
            username: req.user.username,
            action: 'flag/unflag/removeFromIncident',
            actionRef: {
              reportContent: report.content,
              reporttID: report._id,
              referer: referer,
          ...
          console.log("about to call logger");
          logger.logActivity(logDetails);
          res.send(200);
        }
      });
  ...
  app.patch('/api/v1/report/_link', user.can('edit data'), function(req, res) {
    ...
    Report.find({ _id: { $in: req.body.ids } }, function(err, reports) {
      ...
      reports.forEach(function(report) {
        ...
        report.save(function(err) {
          if (err) return res.send(err.status, err.message);
          // TODO Add incident title here somehow, if we can access it from within here
          var logDetails = {
            userID: req.user._id,
            username: req.user.username,
            action: 'addToIncident',
            actionRef: {
              reportContent: report.content,
              reporttID: report._id,
              incidentID: report._incident,
              referer: referer
            }
          };
          logger.logActivity(logDetails);
