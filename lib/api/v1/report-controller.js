...
module.exports = function(app, user) {
  ...
  // Cancel batch
  app.put('/api/v1/report/batch/cancel', user.can('edit reports'), function(req, res) {
    Report.cancelBatch(req.body.id, function(err) {
      if (err) res.send(err.status, err.message);
      else res.send(200);
    });
  });

  // Grab new batch for given user id
  app.get('/api/v1/report/batch/:_id', user.can('edit reports'), function(req, res) {
    Report.checkoutBatch(req.params._id, function(err, reports) {
      if (err) res.send(err.status, err.message);
  ...
  app.post('/api/v1/report/_read', user.can('edit reports'), function(req, res) {
  ...
  app.post('/api/v1/report/_flag', user.can('edit reports'), function(req, res) {
    if (!req.body.ids || !req.body.ids.length) return res.send(200);
