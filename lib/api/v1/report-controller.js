...
module.exports = function(app, user) {
  ...
  app.put('/api/v1/report/:_id', user.can('edit data'), function(req, res) {
    ...
    Report.findById(req.params._id, function(err, report) {
      ...
      _.each(_.pick(req.body, ['flagged', 'escalated', '_incident', 'read', 'smtcTags']), function(val, key) {
        report[key] = val;
      });
      // Save report
      report.save(function(err, numberAffected) {
        if (err) res.send(err.status, err.message);
          ...
          writelog.writeReport(req, report, 'flag/unflag/removeFromIncident/escalated/updateTags');
          res.send(200);
        }
      });
    });
