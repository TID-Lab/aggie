...
module.exports = function(app, user) {
  ...
  app.get('/api/v1/user/:_id', user.can('admin users'), function(req, res) {
    User.findById(req.params._id, '-password', function(err, u) {
      if (err) res.send(err.status, err.message);
      else if (!u) res.send(404);
      else res.send(200, u);
    });
  });
  ...
  app.put('/api/v1/user/:_id', user.can('update users'), function(req, res) {
    User.findById(req.params._id, function(err, user) {
      ...
      if (req.user && req.user.role !== 'admin' && req.params._id != req.user._id) return res.send(403);
