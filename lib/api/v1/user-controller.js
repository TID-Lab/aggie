...
module.exports = function(app, user) {
  ...
  // Get a User by id
  app.get('/api/v1/user/:username', user.can('manage users'), function(req, res) {
  ...
  // Update a User
  app.put('/api/v1/user/:username', user.can('manage users'), function(req, res) {
  ...
  // Delete a User
  app.delete('/api/v1/user/:username', user.can('manage users'), function(req, res) {
    User.findOne({username: req.params.username}, function(err, user) {
      if (err) return res.send(err.status, err.message);
      if (!user) return res.send(404);
