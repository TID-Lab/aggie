...
module.exports = function(app, user) {
  ...
  app.get('/api/v1/incident', user.can('view data'), function(req, res) {
    ...
    // Use paginated find
    Incident.queryIncidents(incidentData, req.query.page, {sort: '-updatedAt', populate: '_assignedTo', populateKeys: 'username' }, function(err, incidents) {
      if (err) res.send(err.status, err.message);
  ...
  app.get('/api/v1/incident/:_id', user.can('view data'), function(req, res) {
    Incident.findById(req.params._id)
