...
module.exports = function (app, user) {
  ...
  app.get("/api/v1/viz", async function (req, res) {
    ...
    data.authors_read = await authorCollection.find({'read_only': true}).sort({'reportCount': -1}).limit(50).toArray();
    ...
    data.media_read = await mediaCollection.find({'read_only': true}).sort({'count': -1}).toArray();
    ...
    data.words_read = await wordCollection.find({'read_only': true}).sort({'count': -1}).limit(200).toArray();

    // Time Visualization
    var timeCollection = aggie.collection('timeVisualization');
    data.time = await timeCollection.find({'read_only': false}).toArray();
    data.time_read = await timeCollection.find({'read_only': true}).toArray();
