...
module.exports = function(app, user) {
  ...
  app.get('/api/user', user.can('view users'), function(req, res) {
    var query = {};
  ...
  // Get a User by id
  app.get('/api/user/:_id', user.can('admin users'), function(req, res) {
  ...
  // Create a new User
  app.post('/api/user', user.can('admin users'), function(req, res) {
  ...
  // Update a User
  app.put('/api/user/:_id', user.can('update users'), function(req, res) {
  ...
  // Delete a User
  app.delete('/api/user/:_id', user.can('admin users'), function(req, res) {
    User.findById(req.params._id, function(err, user) {
      if (err) return res.send(err.status, err.message);
      if (!user) return res.send(404);
